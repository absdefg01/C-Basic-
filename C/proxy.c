    ...
    int main(){
        ...
        hints.ai_family = AF_INET;        // seules les adresses IPv4 seront présentées par
         ...
         strcpy(buffer, "220\n");
            ...
            ecode = read (descSockCOM,buffer,sizeof(buffer));
            buffer[ecode] = '\0';
            printf("Message reçu est : %s\n",buffer);
             
            sscanf(buffer,"%[^@]@%s",var1,var2);
            strcat(var1,"\n");
            strcat(var2,"\n");
            printf("Var1: %sVar2: %s", var1, var2);
     
      //Récupération des informations sur le serveur
      serveuracces = getaddrinfo(serverAddr,serverPort,&hints,&res);
      if (serveuracces){
          fprintf(stderr,"getaddrinfo: %s\n", gai_strerror(serveuracces));
          exit(3);
      }
            //Connection au serveur
      serveuracces = connect(descSockServ, res->ai_addr, res->ai_addrlen);
      if (serveuracces == -1) {
          close(descSockServ);
          freeaddrinfo(res);
      ...
      serveuracces = read(descSockServ, buffer, MAXBUFFERLEN);
      if (serveuracces == -1) {
            perror("Problème de lecture\n");
             exit(6);}
      buffer[serveuracces] = '\0';
